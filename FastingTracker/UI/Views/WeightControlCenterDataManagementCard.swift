...
struct WeightControlCenterDataManagementCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: DSSpacing.cardPadding) {
            ...
            if let status = viewModel.lastExportStatus {
                statusRow(icon: "square.and.arrow.up", text: status, accessibilityPrefix: "Last export:")
            }

            if let status = viewModel.lastImportStatus {
                statusRow(icon: "square.and.arrow.down", text: status, accessibilityPrefix: "Last import:")
            }

            if let status = viewModel.lastDeleteStatus {
                statusRow(icon: "trash", text: status, accessibilityPrefix: "Last delete:")
            }
        }
    ...
    private func statusRow(icon: String, text: String, accessibilityPrefix: String) -> some View {
        HStack(spacing: DSSpacing.cardSmallSpacing) {
            Image(systemName: icon)
                .foregroundColor(Theme.ColorToken.textSecondaryOnDark)
            Text(text)
