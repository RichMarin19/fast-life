...
final class WeightSyncCoordinator: WeightSyncCoordinating {
    ...
    func sync(initialImport: Bool) {
        ...
        let incrementalStart = Calendar.current.date(byAdding: .month, value: -1, to: Date()) ?? Date()
        let historicalStart = Calendar.current.date(byAdding: .year, value: -10, to: Date()) ?? Date()
        let startDate = initialImport ? historicalStart : incrementalStart
