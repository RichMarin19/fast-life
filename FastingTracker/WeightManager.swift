...
class WeightManager: ObservableObject {
    ...
    init() {
        ...
        if self.syncWithHealthKit, HealthKitManager.shared.isWeightAuthorized() {
        ...
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(self.handleHealthKitWeightDeletions(_:)),
    ...
    func addWeightEntry(_ entry: WeightEntry) {
        ...
        if self.syncWithHealthKit, entry.source == .manual {
            ...
            HealthKitManager.shared.saveWeight(
                weight: entry.weight,
                bmi: entry.bmi,
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 2.0) {
                    ...
                    AppLogger.info(
                        "Observer suppression lifted after manual entry sync",
                ...
                if !success {
                    AppLogger.error(
                        "Failed to sync weight to HealthKit",
                        category: AppLogger.weightTracking,
                        error: error
                    )
                } else {
    ...
    func deleteWeightEntry(_ entry: WeightEntry) {
        ...
        if self.syncWithHealthKit {
            AppLogger.info(
                "Bidirectional deletion: removing weight entry from HealthKit (source: \(entry.source))",
                category: AppLogger.weightTracking
            )
            ...
            if let healthKitUUID = entry.healthKitUUID {
                ...
                HealthKitManager.shared.deleteWeightByUUID(healthKitUUID) { success, error in
                    if !success {
                        AppLogger.error(
                            "Failed to delete HealthKit sample by UUID during bidirectional sync",
                            category: AppLogger.weightTracking,
                            error: error
                        )
                        ...
                        AppLogger.info(
                            "Successfully deleted HealthKit sample by UUID via bidirectional sync",
                            category: AppLogger.weightTracking
                        )
                    }
                }
                ...
                AppLogger.info(
                    "No stored UUID - querying HealthKit to find sample for precise deletion",
                    category: AppLogger.weightTracking
                )
                ...
                HealthKitManager.shared.findWeightSampleUUID(date: entry.date, weight: entry.weight) { uuid in
                    if let foundUUID = uuid {
                        AppLogger.info(
                            "Found HealthKit UUID via query - proceeding with precise deletion",
                            category: AppLogger.weightTracking
                        )
                        HealthKitManager.shared.deleteWeightByUUID(foundUUID) { success, error in
                            if !success {
                                AppLogger.error(
                                    "Failed to delete HealthKit sample by queried UUID",
                                    category: AppLogger.weightTracking,
                                    error: error
                                )
                            } else {
                                AppLogger.info(
                                    "Successfully deleted HealthKit sample using queried UUID",
                                    category: AppLogger.weightTracking
                                )
                            }
                        }
                        ...
                        AppLogger.warning(
                            "Could not find matching HealthKit sample for deletion",
                            category: AppLogger.weightTracking
                        )
    ...
    func displayWeight(for entry: WeightEntry) -> Double {
        self.appSettings.weightUnit.fromPounds(entry.weight)
    }
    ...
    func convertWeightToDisplayUnit(_ weightInPounds: Double) -> Double {
        self.appSettings.weightUnit.fromPounds(weightInPounds)
    }
    ...
    func addWeightEntryInPreferredUnit(
        weight: Double,
        bmi: Double? = nil,
        bodyFat: Double? = nil,
        date: Date = Date()
    ) {
        let weightInPounds = self.appSettings.weightUnit.toPounds(weight)
        ...
        guard !isDuplicate else {
            AppLogger.warning(
                "Prevented duplicate weight entry: \(weight) \(self.appSettings.weightUnit.abbreviation) within 30 minutes",
                category: AppLogger.weightTracking
            )
            return
        }
        ...
        AppLogger.info(
            "Adding weight: \(weight) \(self.appSettings.weightUnit.abbreviation) (\(weightInPounds) lbs internal)",
            category: AppLogger.weightTracking
        )
    ...
    func wouldCreateDuplicate(weight: Double, date: Date = Date()) -> Bool {
        let weightInPounds = self.appSettings.weightUnit.toPounds(weight)
        return self.weightEntries.contains(where: {
            // FIXED: Check across ALL sources, not just manual (Industry standard pattern)
    ...
    func syncFromHealthKit(startDate: Date? = nil, completion: ((Int, Error?) -> Void)? = nil) {
        ...
        HealthKitManager.shared.fetchWeightData(startDate: start, resetAnchor: false) { [weak self] healthKitEntries in
            guard let self = self else {
                completion?(
                    ...
                    NSError(
                        domain: "WeightManager",
                        code: 2,
                        userInfo: [NSLocalizedDescriptionKey: "WeightManager instance deallocated"]
                    )
                )
                return
            }
            ...
            DispatchQueue.main.async {
                ...
                AppLogger.info(
                    "HealthKit sync completed: \(newlyAddedCount) new weight entries added",
                    category: AppLogger.weightTracking
                )
            ...
            DispatchQueue.main.async {
                ...
                for hkEntry in healthKitEntries {
                    ...
                    let isDuplicate = self.weightEntries.contains(where: {
                        ...
                        abs($0.date.timeIntervalSince(hkEntry.date)) < 300 &&
                            // Within 5 minutes (more flexible for historical)
                ...
                // Report actual sync results
                AppLogger.info(
                    "Historical HealthKit sync completed: \(newlyAddedCount) new weight entries imported from \(healthKitEntries.count) total entries",
                    category: AppLogger.weightTracking
                )
    ...
    func syncFromHealthKitWithReset(startDate: Date, completion: @escaping (Int, Error?) -> Void) {
        ...
        AppLogger.info(
            "Starting manual sync with anchor reset for deletion detection from \(startDate)",
            category: AppLogger.weightTracking
        )

        HealthKitManager.shared
            .fetchWeightData(startDate: startDate, resetAnchor: true) { [weak self] healthKitEntries in
                guard let self = self else {
                    completion(
                        0,
                        NSError(
                ...
                DispatchQueue.main.async {
                    ...
                    AppLogger.info(
                        "DELETION CHECK: Starting with \(originalCount) Fast LIFe entries, \(healthKitEntries.count) HealthKit entries",
                        category: AppLogger.weightTracking
                    )
                    ...
                    self.weightEntries.removeAll { fastLifeEntry in
                        ...
                        guard fastLifeEntry.source != .manual else {
                            AppLogger.info(
                                "PRESERVING manual entry: \(fastLifeEntry.weight)lbs on \(formatter.string(from: fastLifeEntry.date))",
                                category: AppLogger.weightTracking
                            )
                            return false
                        }
                        ...
                        if !stillExistsInHealthKit {
                            AppLogger.info(
                                "DELETING entry: \(fastLifeEntry.weight)lbs on \(fastLifeDateString) (source: \(fastLifeEntry.source.rawValue)) - not found in current HealthKit data",
                                category: AppLogger.weightTracking
                            )
                        } else {
                            AppLogger.info(
                                "KEEPING entry: \(fastLifeEntry.weight)lbs on \(fastLifeDateString) (source: \(fastLifeEntry.source.rawValue)) - still exists in HealthKit",
                                category: AppLogger.weightTracking
                            )
                        }
                    ...
                    AppLogger.info(
                        "DELETION COMPLETE: Removed \(deletedCount) entries, \(self.weightEntries.count) entries remaining",
                        category: AppLogger.weightTracking
                    )
                    ...
                    var addedCount = 0
                    AppLogger.info(
                        "Starting comparison: HealthKit has \(healthKitEntries.count) entries, Fast LIFe has \(self.weightEntries.count) entries",
                        category: AppLogger.weightTracking
                    )
                    ...
                    for healthKitEntry in healthKitEntries {
                        ...
                        let alreadyExists = self.weightEntries.contains { fastLifeEntry in
                            ...
                            if matches {
                                AppLogger.info(
                                    "MATCH FOUND: HealthKit(\(healthKitEntry.weight)lbs \(healthKitDateString)) matches Fast LIFe(\(fastLifeEntry.weight)lbs \(fastLifeDateString)) - timeDiff:\(timeDiff)s weightDiff:\(weightDiff)lbs",
                                    category: AppLogger.weightTracking
                                )
                            }
                        ...
                        if !alreadyExists {
                            AppLogger.info(
                                "MISSING ENTRY DETECTED: Adding HealthKit entry \(healthKitEntry.weight)lbs on \(healthKitDateString) (source: \(healthKitEntry.source.rawValue))",
                                category: AppLogger.weightTracking
                            )
                            ...
                            AppLogger.info(
                                "Entry already exists: \(healthKitEntry.weight)lbs on \(healthKitDateString)",
                                category: AppLogger.weightTracking
                            )
                        }
                    }
                    ...
                    AppLogger.info(
                        "Manual HealthKit sync completed: \(addedCount) entries added, \(deletedCount) entries removed, \(healthKitEntries.count) total HealthKit entries",
                        category: AppLogger.weightTracking
                    )
    ...
    func setSyncPreference(_ enabled: Bool) {
        ...
        if enabled {
            ...
            AppLogger.info(
                "Weight-specific HealthKit authorization status: \(isAuthorized ? "granted" : "denied")",
                category: AppLogger.weightTracking
            )
            ...
            if !isAuthorized {
                ...
                HealthKitManager.shared.requestWeightAuthorization { success, error in
                    if success {
                        AppLogger.info(
                            "Weight-specific HealthKit authorization granted, setting up observer",
                            category: AppLogger.weightTracking
                        )
                        ...
                        AppLogger.error(
                            "Weight-specific HealthKit authorization failed",
                            category: AppLogger.weightTracking,
                            error: error
                        )
    ...
    private func setupHealthKitObserver() {
        ...
        guard self.syncWithHealthKit, HealthKitManager.shared.isWeightAuthorized() else {
            AppLogger.info(
                "Weight observer not set up - sync disabled or not authorized for weight data",
                category: AppLogger.weightTracking
            )
            return
        }
        ...
        let query = HKObserverQuery(
            ...
            guard let self = self, !self.isSuppressingObserver else {
                AppLogger.info(
                    "HealthKit observer suppressed during manual operation - skipping sync",
                    category: AppLogger.weightTracking
                )
                completionHandler()
                return
            ...
            AppLogger.info(
                "New weight data detected in HealthKit, syncing with deletion support",
                category: AppLogger.weightTracking
            )
            DispatchQueue.main.async {
        ...
        AppLogger.info(
            "Weight HealthKit observer started successfully - automatic sync enabled",
            category: AppLogger.weightTracking
        )
    }
    ...
    var latestWeight: WeightEntry? {
        self.weightEntries.first
    }

    var weightTrend: Double? {
        guard self.weightEntries.count >= 2 else { return nil }
    ...
    var averageWeight: Double? {
        guard !self.weightEntries.isEmpty else { return nil }

        // Optimized: use lazy to avoid creating intermediate array
        let sum = self.weightEntries.lazy.map(\.weight).reduce(0.0, +)
        return sum / Double(self.weightEntries.count)
    }
    ...
    func weightChange(since date: Date) -> Double? {
        ...
        for entry in self.weightEntries.reversed() {
    ...
    private func saveWeightEntries() {
        if let encoded = try? JSONEncoder().encode(weightEntries) {
            self.userDefaults.set(encoded, forKey: self.weightEntriesKey)
        }
    }
    ...
    private func loadWeightEntries() {
        ...
        self.weightEntries = entries.sorted { $0.date > $1.date }
    }
    ...
    @objc private func handleHealthKitWeightDeletions(_ notification: Notification) {
        ...
        AppLogger.info(
            "Processing \(deletedSamples.count) deleted weight entries from HealthKit",
            category: AppLogger.weightTracking
        )
        ...
        for deletedSample in deletedSamples {
            ...
            self.weightEntries.removeAll { entry in
        ...
        if deletedCount > 0 {
            self.saveWeightEntries()
            AppLogger.info(
                "Removed \(deletedCount) weight entries deleted from HealthKit",
                category: AppLogger.weightTracking
            )
        }
    }
