...
## Guiding Principles
- **Single canonical store**: Persist pounds internally (HealthKit alignment) but never reconvert pounds→kilograms unless the measurement provider explicitly formats for display.
- **Measurement awareness everywhere**: Every surface (onboarding, `WeightTrackingView`, `WeightManager`) must ask `MeasurementSystemProviding` which unit to expect/render instead of assuming `.us`.
...
1. **Add scoped logging** (Debug builds only) in `FirstTimeWeightSetupView.saveAndContinue()` and `WeightTrackingView.onAppear`:
...
1. **Inject dependencies**: Provide `WeightDependencies` (or lighter `MeasurementSystemProviding` + `WeightManaging`) into onboarding so the measurement provider is identical between onboarding and the tracker.
2. **Unit-aware inputs**:
...
4. **No conversion when not needed**:
   - If the measurement system is already metric, `WeightTrackingView` must request `WeightManager.formattedDisplayWeight` which converts pounds→kg for display, guaranteeing the tracker shows 82.1 kg immediately.
...
2. **CurrentWeightCard / StartWeight components** render via the shared formatter so flipping the device units updates values in place.
3. **Add regression tests** (or preview harness) to cover:
...
2. Update `docs/handoffs/HANDOFF.md` with the root-cause summary and QA evidence paths.
3. Refresh guardrail baselines if new files were added for DI/measurement helpers.
